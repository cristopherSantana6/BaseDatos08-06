...
-- 83. Insertar 20 clientes (Ejemplo de formato, repetir o ajustar datos)
INSERT INTO TCliente (cNombre, cApellido, cEmail) VALUES 
('Juan', 'Pérez', 'j.perez@email.com'), ('Ana', 'Gómez', 'a.gomez@email.com'), ... ;

-- 84. Registrar 50 ventas (Ejemplo)
INSERT INTO TVenta (nClienteID, nTotal, cMetodoPago) VALUES (1, 150.50, 'Tarjeta');

-- 85. Actualizar montos (Ejemplo: aplicar descuento del 10% a ventas de 'Efectivo')
...
-- 87. Top 5 clientes con mayores compras
SELECT TOP 5 C.cNombre, C.cApellido, SUM(V.nTotal) AS TotalComprado
FROM TCliente C JOIN TVenta V ON C.nClienteID = V.nClienteID
GROUP BY C.cNombre, C.cApellido
ORDER BY TotalComprado DESC;

-- 88. Ventas por mes
SELECT MONTH(dFechaVenta) AS Mes, SUM(nTotal) AS VentasTotales
FROM TVenta GROUP BY MONTH(dFechaVenta);

-- 89. Promedio de ventas por cliente
SELECT nClienteID, AVG(nTotal) AS PromedioVenta
FROM TVenta GROUP BY nClienteID;

-- 90. Reporte consolidado (Uniendo Cliente, Venta y Detalle - asumiendo una tabla Producto/Detalle)
SELECT C.cNombre, C.cApellido, V.nVentaID, V.nTotal, V.dFechaVenta
FROM TCliente C
